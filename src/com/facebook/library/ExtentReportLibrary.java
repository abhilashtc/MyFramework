...
public class ExtentReportLibrary {
	...
	public void captureStatus(ITestResult result, WebDriver driver) throws Exception {
		...
		String nodeJS = GeneralUtilities.getValueOfConfigProp("Jira.properties", "nodeJS");
		...
		if(result.getStatus() == ITestResult.SUCCESS) {
			...
			System.out.println("TestBase.lastWebElement -> " + TestBase.lastWebElement);
			if (TestBase.lastWebElement != null)
				GeneralUtilities.highlightElement(driver, TestBase.lastWebElement);
//			String screenshotPath = "file:///" + captureScreen(result.getName(),driver);
			String screenshotPath = nodeJS + captureScreen(result.getName(),driver);
			testInfo.fail("Screen Shot " + testInfo.addScreenCaptureFromPath(screenshotPath));
			
			if (TestBase.openJIRA_Ticket == true) {
				JIRA_Ticket.testCaseName = result.getName();
				JIRA_Ticket.createJiraIssue(screenshotPath);
				testInfo.log(Status.INFO, "Opened Jira Ticket... " + TestBase.JIRA_Ticket);
				TestBase.lastWebElement = null;
			}
			...
			if (TestBase.lastWebElement != null)
				GeneralUtilities.highlightElement(driver, TestBase.lastWebElement);
			String screenshotPath = nodeJS + captureScreen(result.getName(),driver);
			testInfo.skip("Screen Shot " + testInfo.addScreenCaptureFromPath(screenshotPath));
		}
	}
